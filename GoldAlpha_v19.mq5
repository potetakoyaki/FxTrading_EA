//+------------------------------------------------------------------+
//| GoldAlpha_v19.mq5 - WFA-Robust v12 base + EMA Slope             |
//| Strategy: v12 base + optimized entry/exit + EMA slope filter     |
//| Changes from v12:                                                |
//|   - SL_ATR_Mult: 2.0 -> 3.1 (wider SL, let winners run)        |
//|   - Trail_ATR: 2.5 -> 3.5 (wider trailing)                      |
//|   - BE_ATR: 1.5 -> 0.8 (faster breakeven)                       |
//|   - EMA_Zone_ATR: 0.4 -> 0.65 (wider dip zone)                  |
//|   - ATR_Filter: 0.6 -> 0.7 (higher vol threshold)               |
//|   - BodyRatio: 0.32 -> 0.24 (accept more candle patterns)       |
//|   - D1_Tolerance: 0.003 -> 0.002 (tighter D1 alignment)         |
//|   - MaxPositions: 2 -> 3                                         |
//|   - NEW: EMA Slope(5) filter for trend alignment                 |
//| Results: PF=2.01, 1082T, DD=25.7%, WR=66.6%                     |
//|   OOS 2024-2026: PF=3.31, 263T, DD=12.3%                        |
//|   OOS Risk=2.5%: Daily=7,303 JPY on 300K JPY                    |
//+------------------------------------------------------------------+
...
// --- Risk ---
input double   SL_ATR_Mult   = 3.1;
input double   Trail_ATR     = 3.5;
input double   BE_ATR        = 0.8;
...
input double   D1_Tolerance  = 0.002;
...
{
   double emaCur[], emaPrev[];
   if(CopyBuffer(hH4EMA, 0, 1, 1, emaCur) < 1) return false;
   if(CopyBuffer(hH4EMA, 0, EMA_Slope_Bars + 1, 1, emaPrev) < 1) return false;
   double slope = emaCur[0] - emaPrev[0];
   if(direction == 1 && slope < 0) return false;
   if(direction == -1 && slope > 0) return false;
   return true;
}
...
void OnTick()
{
   ...
   if(w1Dir == 1)
   {
      if(CheckBuyDip(1, h4ema[0], zone) || CheckBuyDip(2, h4ema[0], zone))
      {
         ...
         trade.Buy(lot, _Symbol, ask, sl, 0, "Alpha19 BUY");
      }
   }
   ...
   if(w1Dir == -1)
   {
      if(CheckSellDip(1, h4ema[0], zone) || CheckSellDip(2, h4ema[0], zone))
      {
         ...
         double lot = CalcLot(slDist);
         trade.Sell(lot, _Symbol, bid, sl, 0, "Alpha19 SELL");
      }
   }
}
//+------------------------------------------------------------------+
