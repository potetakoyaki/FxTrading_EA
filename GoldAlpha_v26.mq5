//+------------------------------------------------------------------+
//| GoldAlpha_v26.mq5 - Adaptive H4 Slope + Tighter Range Control   |
//| v25 base + adaptive slope based on D1 trend strength             |
//| + tighter SL in weak trends to reduce losses                     |
//+------------------------------------------------------------------+
...
input double   D1_Strong_Slope = 0.004; // Above this = strong trend
input double   W1_Min_Sep     = 0.007;
input int      H4_Slope_Strong = 8;    // H4 slope bars in strong trend
input int      H4_Slope_Weak   = 3;    // H4 slope bars in weak trend
input double   MinLot        = 0.01;
input double   MaxLot        = 0.15;
input int      MagicNumber   = 330026;
...
void OnTick()
{
   ...
   if(d1slope < D1_Min_Slope) return;  // Skip ranging
   ...
   // Adaptive SL: tighter in weak trends
   ...
   {double sd=slMult*av;trade.Buy(CalcLot(sd),_Symbol,ask,NormalizeDouble(ask-sd,_Digits),0,"A26 BUY");}
   if(dir==-1&&(CheckSellDip(1,h4e[0],zone)||CheckSellDip(2,h4e[0],zone)))
   {double sd=slMult*av;trade.Sell(CalcLot(sd),_Symbol,bid,NormalizeDouble(bid+sd,_Digits),0,"A26 SELL");}
}
