//+------------------------------------------------------------------+
//| GoldAlpha_v32.mq5 - Stability-First: Adaptive Risk + Single Pos |
//| v28 base with regime-adaptive risk management:                   |
//|   - MaxPositions=1 (no concurrent exposure)                      |
//|   - Adaptive risk: full risk in strong D1, half in weak D1       |
//|   - Tighter SL in weak trends (adaptive SL retained)            |
//| Goal: Smooth, steadily rising equity curve from 300K JPY         |
//+------------------------------------------------------------------+
...
input double   RiskPct       = 1.0;       // Base risk (strong D1)
input double   WeakRiskScale = 0.5;       // v32: risk x0.5 in weak D1
input double   BodyRatio     = 0.32;
input double   EMA_Zone_ATR  = 0.50;
input double   ATR_Filter    = 0.55;
input double   D1_Tolerance  = 0.003;
input int      MaxPositions  = 1;         // v32: single position only
...
void OnTick()
{
   ...
   // v32: Adaptive risk - half risk in weak trends
   ...
   {double sd=slMult*av;trade.Buy(CalcLot(sd,effectiveRisk),_Symbol,ask,NormalizeDouble(ask-sd,_Digits),0,"A32 BUY");}
   if(dir==-1&&(CheckSellDip(1,h4e[0],zone)||CheckSellDip(2,h4e[0],zone)))
   {double sd=slMult*av;trade.Sell(CalcLot(sd,effectiveRisk),_Symbol,bid,NormalizeDouble(bid+sd,_Digits),0,"A32 SELL");}
}
