//+------------------------------------------------------------------+
//| GoldAlpha_v20.mq5 - WFA-Robust with D1 Slope Regime Filter      |
//| Strategy: v19 base + D1 EMA slope regime detection               |
//| Changes from v19:                                                |
//|   - NEW: D1 EMA slope filter (skip ranging markets)              |
//|     Skip entry if D1 EMA hasn't moved >= 0.2% over 5 bars       |
//|   - This fixes WFA from 3/8 -> 5/8                              |
//| Results: PF=2.23, 808T, DD=21.0%, WR=65.6%                      |
//|   WFA: 5/8 PASS (vs v19's 3/8)                                  |
//|   OOS 2024-2026 Risk=2.5%: Daily=6,838 JPY on 300K JPY          |
//+------------------------------------------------------------------+
...
void OnTick()
{
   ...
   if(w1Dir == 1)
   {
      if(CheckBuyDip(1, h4ema[0], zone) || CheckBuyDip(2, h4ema[0], zone))
      {
         ...
         trade.Buy(lot, _Symbol, ask, sl, 0, "Alpha20 BUY");
      }
   }
   ...
   if(w1Dir == -1)
   {
      if(CheckSellDip(1, h4ema[0], zone) || CheckSellDip(2, h4ema[0], zone))
      {
         ...
         double lot = CalcLot(slDist);
         trade.Sell(lot, _Symbol, bid, sl, 0, "Alpha20 SELL");
      }
   }
}
//+------------------------------------------------------------------+
