//+------------------------------------------------------------------+
//| GoldAlpha_v12.mq5 - Multi-position with v9 quality              |
//| Strategy: v9 base + allow 2 concurrent positions                |
//| + moderate filter relaxation to increase frequency               |
//| Changes:                                                         |
//|   - MaxPositions: 2 (was 1) - 2nd entry on deeper dip           |
//|   - EMA_Zone_ATR: 0.4 (moderate, between v9 0.3 and v10 0.5)   |
//|   - BodyRatio: 0.32 (moderate)                                  |
//|   - ATR_Filter: 0.6 (moderate, between v9 0.7 and v10 0.5)     |
//|   - Check bar 1 and 2                                           |
//|   - Risk 0.18% per position (0.36% max with 2 open)             |
//|   - MaxLot 0.10 for safety with multiple positions              |
//|   - D1 tolerance 0.3%                                           |
//+------------------------------------------------------------------+
...
input double   RiskPct       = 0.18;
input double   BodyRatio     = 0.32;

// --- Entry Filters ---
input double   EMA_Zone_ATR  = 0.4;
input double   ATR_Filter    = 0.6;
...
void OnTick()
{
   ...
   if(w1Dir == 1)
   {
      if(CheckBuyDip(1, h4ema[0], zone) || CheckBuyDip(2, h4ema[0], zone))
      {
         ...
         trade.Buy(lot, _Symbol, ask, sl, 0, "Alpha12 BUY");
      }
   }
   ...
   if(w1Dir == -1)
   {
      if(CheckSellDip(1, h4ema[0], zone) || CheckSellDip(2, h4ema[0], zone))
      {
         ...
         double lot = CalcLot(slDist);
         trade.Sell(lot, _Symbol, bid, sl, 0, "Alpha12 SELL");
      }
   }
}
//+------------------------------------------------------------------+
