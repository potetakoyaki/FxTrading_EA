//+------------------------------------------------------------------+
//| GoldAlpha_v6.mq5 - 816 trades, PF=1.66, DD=12.75% (2018-2026)  |
//| Strategy: W1/D1 trend + H4 EMA pullback bounce + 2 positions    |
//| Key parameters:                                                  |
//|   - MaxPositions: 2 (concurrent entries on dips)                |
//|   - EMA_Zone_ATR: 0.7 (wide dip detection zone)                |
//|   - ATR_Filter: 0.4 (captures low-vol pullbacks)                |
//|   - BodyRatio: 0.32 (moderate quality filter)                   |
//|   - Risk: 0.24% per trade, MaxLot=0.15                         |
//|   - D1 tolerance: 0.3% for near-EMA entries                    |
//| Full period: PF=1.66, 816 trades, EqDD=12.75%, WR=56.68%      |
//| OOS 2022-2026: PF=1.86, 420 trades, EqDD=14.26%                |
//+------------------------------------------------------------------+
...
input double   RiskPct       = 0.24;
input double   BodyRatio     = 0.32;

// --- Entry Filters ---
input double   EMA_Zone_ATR  = 0.7;
input double   ATR_Filter    = 0.4;
...
void OnTick()
{
   ...
   // D1: relaxed filter with tolerance
   double d1ema[1];
   ...
   if(w1Dir == 1)
   {
      if(CheckBuyDip(1, h4ema[0], zone) || CheckBuyDip(2, h4ema[0], zone))
      {
         ...
         trade.Buy(lot, _Symbol, ask, sl, 0, "GoldAlpha6 BUY");
      }
   }
   ...
   if(w1Dir == -1)
   {
      if(CheckSellDip(1, h4ema[0], zone) || CheckSellDip(2, h4ema[0], zone))
      {
         ...
         double lot = CalcLot(slDist);
         trade.Sell(lot, _Symbol, bid, sl, 0, "GoldAlpha6 SELL");
      }
   }
}
//+------------------------------------------------------------------+
