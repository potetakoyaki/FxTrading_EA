//+------------------------------------------------------------------+
//|                          AntigravityMTF_EA_Gold_v17.mq5          |
//|            ゴールド(XAUUSD)専用 マルチタイムフレーム EA             |
//|            v17.0: Ultimate Edition - Modular Risk + ML Regime     |
//|                   + JSON Config. All new features TOGGLEABLE.     |
//|                   When toggles OFF = identical to v16.0.          |
//+------------------------------------------------------------------+
...
#property description "XAUUSD専用 v17.0 - Ultimate Edition: Modular Risk + ML Regime + JSON Config"

#include <Trade/Trade.mqh>

// v17.0: Modular includes (toggleable via input parameters)
#include <AntigravityMTF_Config.mqh>
#include <AntigravityMTF_Risk.mqh>
...
input group "=== 半利確 ==="
input bool   UsePartialClose   = false;    // v16: Python同期 (50% TP距離でパーシャルクローズ) [Ablation: OFF]
...
input group "=== v17.0 モジュラー拡張 ==="
input bool   UseModularRisk = true;    // v17: カスケード型リスク検証モジュール使用
input bool   UseRegimeML    = true;    // v17: MLレジーム検出使用 [Ablation: ON]
input bool   UseJSONConfig  = false;   // v17: JSONファイルからパラメータ読込
input string ConfigFile     = "AntigravityMTF_Config.json"; // v17: JSON設定ファイル名
...
// v17.0: Modular module instances (always declared, conditionally used via toggle inputs)
CAntigravityConfig g_config;           // JSON-based config management
CRiskManager       g_riskMgr;          // Cascading risk & position sizing
CRegimeML          g_regimeML;         // ML regime detection (KNN)
int                g_mlRegimeResult;   // Cached ML regime for current tick
double             g_mlLotMultiplier;  // ML regime lot multiplier
int                g_mlMinScoreAdj;    // ML regime min score adjustment
bool               g_v17ModulesReady;  // Track if v17 modules initialized OK
string             g_v17Version = "v17.0 - Ultimate Edition: Modular Risk + ML Regime + JSON Config";
...
int OnInit()
{
   ...
   // ────────────────────────────────────────────────────────────────
   // v17.0: Modular module initialization
   // ────────────────────────────────────────────────────────────────
   g_v17ModulesReady = true;
   g_mlRegimeResult  = 0;  // REGIME_TREND
   g_mlLotMultiplier = 1.0;
   g_mlMinScoreAdj   = 0;

   // v17.0: JSON Config — Set defaults and optionally load from file
   g_config.SetDefaults();
   if(UseJSONConfig)
   {
      if(g_config.LoadFromFile(ConfigFile))
      {
         Print("[v17.0] JSON config loaded from ", ConfigFile);
         PrintConfigSummary(g_config);
      }
      else
      {
         Print("[v17.0] JSON config not found or failed — using defaults");
      }
   }

   // v17.0: Modular Risk Manager
   if(UseModularRisk)
   {
      if(!g_riskMgr.Init(RiskPercent, MaxLots, MinLots, MaxDrawdownPct, DDHalfRiskPct, DailyMaxLossPct))
      {
         Print("[v17.0] WARNING: Risk manager init failed — falling back to v16 lot sizing");
         g_v17ModulesReady = false;
      }
      else
      {
         g_riskMgr.SetMagicNumber(MagicNumber);
         g_riskMgr.SetSymbol(_Symbol);
         g_riskMgr.LoadState();
         Print("[v17.0] Risk manager initialized successfully");
      }
   }

   // v17.0: ML Regime Detection (experimental)
   if(UseRegimeML)
   {
      // Create H4 ATR handle for ML module (h_h4_adx already exists)
      int h4_atr_handle = iATR(_Symbol, PERIOD_H4, 14);
      if(h4_atr_handle == INVALID_HANDLE)
      {
         Print("[v17.0] WARNING: H4 ATR handle creation failed — ML regime disabled");
         g_v17ModulesReady = false;
      }
      else if(!g_regimeML.Init(h_h4_adx, h4_atr_handle, h_m15_atr, h_h1_rsi, h_h1_bb))
      {
         Print("[v17.0] WARNING: ML regime init failed — falling back to ER-based detection");
         g_v17ModulesReady = false;
      }
      else
      {
         Print("[v17.0] ML regime detector initialized (",
               g_regimeML.GetTrainingSampleCount(), " training samples, ML active: ",
               (g_regimeML.IsMLActive() ? "YES" : "NO (fallback)"), ")");
      }
   }

   Print("AntigravityMTF EA [GOLD] v17.0 Ultimate Edition 初期化完了");
   Print("   ", g_v17Version);
   Print("   v17.0 ModularRisk=", (UseModularRisk ? "有効" : "無効"),
         " RegimeML=", (UseRegimeML ? "有効" : "無効"),
         " JSONConfig=", (UseJSONConfig ? "有効" : "無効"),
         " ModulesReady=", (g_v17ModulesReady ? "OK" : "FALLBACK"));
...
{
   // v17.0: Save modular module state before cleanup
   if(UseModularRisk)
   {
      g_riskMgr.SaveState();
      Print("[v17.0] Risk manager state saved");
   }
   if(UseJSONConfig)
   {
      g_config.SaveToFile(ConfigFile);
      Print("[v17.0] JSON config saved to ", ConfigFile);
   }
...
void OnTick()
{
   ...
   if(dtNow.day != g_lastDay)
   {
      ...
      // v17.0: Daily reset for modular risk manager
      if(UseModularRisk)
         g_riskMgr.OnNewDay();
   }
   if(g_circuitBreaker) return;  // 日次損失上限到達

   // v17.0: Diagnostic panel (Comment overlay on chart)
   if(UseModularRisk || UseRegimeML || UseJSONConfig)
   {
      string diagPanel = "";
      diagPanel += "=== AntigravityMTF " + g_v17Version + " ===\n";
      diagPanel += "Regime: " + g_detailedRegime;
      if(UseRegimeML && g_v17ModulesReady)
      {
         diagPanel += " | ML: " + g_regimeML.GetRegimeName(g_mlRegimeResult)
                    + " (conf=" + DoubleToString(g_regimeML.GetRegimeConfidence(), 2)
                    + ", " + (g_regimeML.IsMLActive() ? "ML" : "Fallback") + ")";
         diagPanel += "\nML LotMult=" + DoubleToString(g_mlLotMultiplier, 2)
                    + " ScoreAdj=+" + IntegerToString(g_mlMinScoreAdj)
                    + " Samples=" + IntegerToString(g_regimeML.GetTrainingSampleCount());
      }
      else
      {
         diagPanel += " (ER-based)";
      ...
      if(UseModularRisk)
      {
         diagPanel += "Risk: DD=" + DoubleToString(g_riskMgr.GetCurrentDD(), 1) + "%"
                    + " Level=" + IntegerToString(g_riskMgr.GetDDEscalationLevel())
                    + " DailyPnL=" + DoubleToString(g_riskMgr.GetDailyPnL(), 2)
                    + (g_riskMgr.IsDailyLimitHit() ? " [CIRCUIT BREAKER]" : "")
                    + (g_riskMgr.IsInCooldown() ? " [COOLDOWN " + IntegerToString(g_riskMgr.CooldownRemaining()) + "m]" : "")
                    + "\n";
         diagPanel += "Trades: " + IntegerToString(g_riskMgr.GetTradeCount())
                    + " WR=" + DoubleToString(g_riskMgr.GetWinRate(30) * 100, 1) + "%"
                    + " Expect=" + DoubleToString(g_riskMgr.GetExpectancy(30), 2)
                    + "\n";
      }
      if(UseJSONConfig)
      {
         diagPanel += "Config: " + ConfigFile + " (sync OK)\n";
      }
      diagPanel += "Positions: " + IntegerToString(CountMyPositions())
                 + "/" + IntegerToString(MaxPyramidPositions)
                 + " VolRatio=" + DoubleToString(g_volRatio, 2)
                 + " ER=" + DoubleToString(g_h4ER, 2);
      Comment(diagPanel);
   }
   ...
   // v17.0: ML Regime Detection Override (when enabled, overrides ER-based detection)
   g_mlRegimeResult  = 0;  // Reset each tick
   g_mlLotMultiplier = 1.0;
   g_mlMinScoreAdj   = 0;
   if(UseRegimeML && g_v17ModulesReady)
   {
      g_mlRegimeResult = g_regimeML.DetectRegime();
      g_mlLotMultiplier = g_regimeML.GetLotMultiplier();
      g_mlMinScoreAdj   = g_regimeML.GetMinScoreAdjust();

      // Map ML regime (0-3) to the string-based regime used by v16 code
      switch(g_mlRegimeResult)
      {
         case 0: // REGIME_TREND
            g_currentRegime = "trend";
            g_detailedRegime = "trend";
            break;
         case 1: // REGIME_RANGE
            g_currentRegime = "range";
            g_detailedRegime = "range";
            break;
         case 2: // REGIME_HIGHVOL
            g_currentRegime = "high_vol";
            g_detailedRegime = "high_vol";
            break;
         case 3: // REGIME_CRASH
            g_currentRegime = "crash";
            g_detailedRegime = "crash";
            break;
         default:
            // Keep ER-based detection result
            break;
      }

      // ML crash detection overrides — no trading
      if(g_mlRegimeResult == 3) return; // REGIME_CRASH
   }
   ...
   // v17.0: Modular risk-based lot sizing (when enabled, replaces v16 CalcLotSize)
   if(UseModularRisk && g_v17ModulesReady)
   {
      // Determine order type for risk calculation
      ENUM_ORDER_TYPE lot_order_type = (buyScore > sellScore) ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
      double lot_entry_price = (lot_order_type == ORDER_TYPE_BUY) ? ask : bid;
      double lot_sl_price, lot_tp_price;
      if(lot_order_type == ORDER_TYPE_BUY)
      {
         lot_sl_price = lot_entry_price - slDist;
         lot_tp_price = lot_entry_price + tpDist;
      }
      else
      {
         lot_sl_price = lot_entry_price + slDist;
      ...
      lot = g_riskMgr.CalcLotSize(_Symbol, lot_order_type, lot_entry_price, lot_sl_price, lot_tp_price);

      // If modular risk rejects the trade, lot=0 and we skip entry
      if(lot <= 0)
      {
         Print("[v17.0] Risk manager rejected trade: ",
               g_riskMgr.GetLastRejectMsg());
         return;
      }
   }
   else
   {
      // v16.0 original lot sizing (fallback)
      lot = CalcLotSize(ask, slDist);
   }

   // v3.0: エクイティカーブフィルター (skip if modular risk already applied equity curve filter)
   if(!UseModularRisk && UseEquityCurveFilter && !IsEquityCurveAboveMA())
   {
   ...
   // v17.0: ML Regime min score adjustment (when enabled)
   if(UseRegimeML && g_v17ModulesReady && g_mlMinScoreAdj > 0)
   {
      currentMinScore = (int)MathMax(currentMinScore, currentMinScore + g_mlMinScoreAdj);
   }
   ...
   // v17.0: ML Regime lot multiplier (when enabled)
   if(UseRegimeML && g_v17ModulesReady && g_mlLotMultiplier < 1.0)
   {
      lot = NormalizeDouble(lot * g_mlLotMultiplier, 2);
      lot = MathMax(MinLots, lot);
   }
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      ...
      if(HistoryDealSelect(trans.deal))
      {
         ...
         if(dealMagic == MagicNumber && dealEntry == DEAL_ENTRY_OUT)
         {
            ...
            // v17.0: Feed trade result to modular risk manager
            if(UseModularRisk)
            {
               g_riskMgr.AddTradeResult(netResult);
               // Start cooldown on SL hit via modular risk manager too
               if(dealReason == DEAL_REASON_SL)
                  g_riskMgr.StartCooldown(CooldownMinutes);
            }

            // v17.0: Feed trade result to ML regime detector for online learning
            if(UseRegimeML && g_v17ModulesReady)
            {
               // Determine profit factor for this trade (simplified)
               double tradePF = 0;
               if(netResult > 0)
                  tradePF = 1.5; // Approximate PF for winning trade
               else if(netResult < 0)
                  tradePF = 0.5; // Approximate PF for losing trade
               g_regimeML.OnTradeResult(g_mlRegimeResult, netResult > 0, tradePF);
            }
